        ...
        <div className="w-full overflow-x-auto relative">
            <Slider {...settings}>
                {trendingCoins.map((coin, index) => (
                    <div key={index} className="w-full md:w-200 h-30 border border-gray-200 rounded-md p-2 flex flex-col items-center justify-between ">
                        <div className='flex flex-row items-center justify-between md:gap-20'>
                        ...
                        <p className="text-sm mt-1 md:mr-24">{`${coin.item.data.price}`}</p>
                        <img src={coin.item.data.sparkline} alt="Sparkline" className="mt-2 w-full md:w-56 h-29" />
                    </div>
                ))}
            </Slider>
        </div>
